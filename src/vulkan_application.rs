use vulkan_tutorial::{
    ...
    utility::share,
    utility::structures::*,
    utility::window::{VulkanApplicationBase},
};
...
impl VulkanApplication {
    pub fn new(event_loop: &winit::event_loop::EventLoop<()>) -> VulkanApplication {
        ...
        let ubo_layout = VulkanApplication::create_descriptor_set_layout(&device);
        ...
        let (uniform_buffers, uniform_buffers_memory) = VulkanApplication::create_uniform_buffers(
            &device,
            &physical_device_memory_properties,
            swapchain_stuff.swapchain_images.len(),
        );
        let descriptor_pool =
            VulkanApplication::create_descriptor_pool(&device, swapchain_stuff.swapchain_images.len());
        let descriptor_sets = VulkanApplication::create_descriptor_sets(
            &device,
            descriptor_pool,
            ubo_layout,
            &uniform_buffers,
...
impl VulkanApplicationBase for VulkanApplication {
